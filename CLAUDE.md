...
| 22 | **No decorative emojis on clinical surfaces (System B / Envision Atlus)** — emojis are a **WellFit Community (System A) design element** and stay there. On any Atlus/admin/clinical surface (`src/components/admin/*`, `bed-board/`, `smart-app/`, `sections/`, and any clinician-facing UI), severity and status are communicated with **EA badge/icon components**, never emoji glyphs. **Exception:** a deliberate, designed *moment of delight* at a meaningful workflow milestone (e.g. the handoff-complete animation nurses see) is allowed — that's a scoped UX reward, not decoration on the data. The ban is on emoji glyphs baked into metrics/forms/headings/`StatCard` icons, not on intentional milestone animations. Reason: emojis read as consumer/toy to a compliance officer or clinician, screen readers announce them literally (⚠️ = "warning sign emoji") for the seniors these very rules target, and they don't localize. The A/B architectural wall in `governance-boundaries.md` now has an **aesthetic** counterpart. | A clinical fall-risk form with `🚽 Bathroom Independence` / `🍽️ Daily Living Activities` headers. A clinical dashboard `StatCard icon="🚨"`. Any emoji in rendered JSX under `src/components/admin/`. (Caught 2026-07-11 — ~300 emoji lines across 47 clinical files, incl. `RiskAssessmentForm.tsx`.) |

| 23 | **Code is truth, docs are the record** — before any work, read the actual source in the work area (docs only say where to look); verify every factual claim about the system by grep/read at HEAD in the same turn; when code and docs disagree, the code wins and correcting the doc is part of the task. | Working from doc/tracker claims without reading the code. Asserting counts or "X exists/works" not verified at HEAD. Leaving a known-false doc claim standing. (Ratified by Maria 2026-08-15.) |

### Documentation Is the Evidence Record — Why Doc Accuracy Is a Safety Requirement

**Maria (2026-08-15):** *"The gaps are filled and documentation is proof of where we were, what mistakes were made, what things we actually fixed and hardened, where we've come from, how we made it here. Those are important — I have a story, and when we need proof that our system is safe."*

Documentation in this repo serves two audiences, and both demand accuracy:

1. **The AI itself.** Docs (PROJECT_STATE, trackers, rules) are the control input the next session acts on. AI sessions don't reliably document everything they did, so docs drift from code — which is why Commandment #23 exists: read the code, don't just trust the map. An unverified doc claim is a wrong instruction to the machine that builds the system.
2. **The safety story.** The docs are the evidentiary record of the hardening journey — every mistake found, every fix shipped, every live proof run. That record is what gets shown to Akima, a SOC 2 assessor, an ONC test lab, a pilot clinic's security review, or a due-diligence team. A system that can *prove* it finds and fixes its defects is more credible than one that claims it never had any.

**Therefore:**
- **Never sanitize history.** Mistakes, incidents, and refuted claims stay in the record with dates and evidence. The caught-and-fixed trail IS the proof of safety.
- **Never let a known-false claim stand.** A doc row that says READY when the code says otherwise is a defect — fix it in the same session it's discovered.
- **Record what you did.** Session close = PROJECT_STATE updated with what actually happened, including what failed. An undocumented fix is a fix the story can't prove.
