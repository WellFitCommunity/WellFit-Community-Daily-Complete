...
export const EnvisionTotpSetupPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const beginSetup = async () => {
      try {
        ...
        // Call envision-totp-setup with action='begin'
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update backup codes if new ones returned
      ...
      // Navigate to the super admin portal after a short delay
      setTimeout(() => {
                ...
                <div className="mt-3 p-4 bg-slate-700/50 rounded-lg space-y-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <code className={`flex-1 text-sm bg-slate-800 px-3 py-2 rounded font-mono text-white break-all ${!showSecret ? 'blur-sm select-none' : ''}`}>
                        {secret}
                      </code>
                      <button
                        type="button"
