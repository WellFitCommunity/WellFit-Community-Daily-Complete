...
const DashMealOfTheDay: React.FC<DashMealOfTheDayProps> = ({ onSeeDetails }) => {
  ...
  return (
    <div className="bg-white text-[#003865] rounded-xl shadow-md p-4 space-y-3">
      ...
      <img
        src={meal.imageUrl}
        alt={meal.name}
        className="rounded-md w-full max-h-52 object-cover"
      />
      <h3 className="text-lg font-semibold">{meal.name}</h3>
      <p>{meal.description}</p>
      <p className="text-sm font-medium">~ {meal.calories} calories</p>
      <button
        className="mt-2 px-4 py-2 bg-[#8cc63f] text-white font-semibold rounded hover:bg-[#003865] transition"
