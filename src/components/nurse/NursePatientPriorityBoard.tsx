/**
 * Nurse Patient Priority Board
 *
 * AI-powered patient prioritization for nurses.
 * Shows ONLY patients assigned to the current nurse (via care_team.nurse_id).
 * Each patient card includes the Patient Avatar thumbnail.
 * Click any patient to open their full chart.
 *
 * Reuses scoring logic from physician/priorityScoring.ts.
 * Dark theme to match the nurse panel.
 */
...
import {
  ...
  type EngagementRow, type CheckInRow, type ProfileRow,
  getLocationScore, getLocationLabel, calculatePriorityScore,
  getPriorityLevel, isAbnormalVitals, buildRiskFlags,
} from '../physician/priorityScoring';

// Dark theme overrides for nurse panel
const DARK_PRIORITY_STYLES: Record<PriorityLevel, { border: string; bg: string; badge: string; text: string }> = {
  critical: { border: 'border-red-500', bg: 'bg-red-950/40', badge: 'bg-red-600 text-white', text: 'text-red-400' },
  high: { border: 'border-orange-500', bg: 'bg-orange-950/40', badge: 'bg-orange-500 text-white', text: 'text-orange-400' },
  moderate: { border: 'border-yellow-500', bg: 'bg-yellow-950/30', badge: 'bg-yellow-500 text-white', text: 'text-yellow-400' },
  low: { border: 'border-green-500', bg: 'bg-green-950/30', badge: 'bg-green-600 text-white', text: 'text-green-400' },
};

// ============================================================================
// PRIORITY CARD — Dark-themed for nurse panel
// ============================================================================
...
const PriorityCard: React.FC<PriorityCardProps> = ({ patient, rank, onSelect }) => {
  const styles = DARK_PRIORITY_STYLES[patient.priority_level];

  return (
    <button
      onClick={() => onSelect(patient.patient_id)}
      className={`w-full text-left rounded-xl border-2 ${styles.border} ${styles.bg} p-4 hover:shadow-lg hover:shadow-black/30 transition-all group`}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-3 text-sm text-slate-400">
            ...
            {patient.latest_hr !== null && (
              <span className={`flex items-center gap-1 ${patient.latest_hr > 120 || patient.latest_hr < 50 ? 'text-red-400 font-bold' : ''}`}>
                <Activity className="w-3.5 h-3.5" /> HR {patient.latest_hr}
              </span>
            )}
            {patient.latest_spo2 !== null && (
              <span className={`flex items-center gap-1 ${patient.latest_spo2 < 92 ? 'text-red-400 font-bold' : ''}`}>
                <Thermometer className="w-3.5 h-3.5" /> SpO2 {patient.latest_spo2}%
              </span>
            )}
            {patient.readmission_risk !== null && (
              <span className={`flex items-center gap-1 ${patient.readmission_risk >= 0.6 ? 'text-red-400 font-bold' : ''}`}>
          ...
          {patient.risk_flags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {patient.risk_flags.map((flag, i) => (
                <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 bg-red-900/50 text-red-300 text-xs rounded-full font-medium">
          ...
          <div className="flex justify-end mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <span className="text-sm text-teal-400 font-medium flex items-center gap-1">
...
const NursePatientPriorityBoard: React.FC = () => {
  ...
  const loadPriorityBoard = useCallback(async () => {
    ...
    try {
      // 1. Get this nurse's assigned patients from care_team (nurse_id)
      ...
      const assignedPatientIds = [...new Set(
        ((assignments || []) as Array<{ patient_id: string }>).map(a => a.patient_id)
      )];

      // Also check care_team_members for nurse role assignments
      const { data: memberAssignments } = await supabase
        .from('care_team_members')
        .select('patient_id')
        .eq('member_id', nurseId)
        .is('end_date', null);

      if (memberAssignments) {
        for (const a of memberAssignments as Array<{ patient_id: string }>) {
          if (!assignedPatientIds.includes(a.patient_id)) {
            assignedPatientIds.push(a.patient_id);
          }
        }
      }
      ...
      // 2. Query all data sources in parallel
      ...
      await auditLogger.info('NURSE_PRIORITY_BOARD_LOADED', {
        nurseId,
      ...
      await auditLogger.error(
        'NURSE_PRIORITY_BOARD_FAILED',
        err instanceof Error ? err : new Error(String(err)),
        { nurseId, component: 'NursePatientPriorityBoard' },
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-8">
        <div className="flex items-center justify-center gap-3">
          ...
          <span className="text-slate-400 font-medium">AI analyzing your patient priorities...</span>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-red-950/30 rounded-xl border border-red-500 p-6">
        <p className="text-red-400 font-medium">Failed to load priority board</p>
        <p className="text-sm text-red-300/60 mt-1">{error}</p>
        <button onClick={loadPriorityBoard} className="mt-3 px-4 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-500">
          Retry
        </button>
      </div>
    );
  ...
  if (counts.total === 0) {
    return (
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-8 text-center">
        <p className="text-slate-300 font-medium">No patients assigned to you</p>
        <p className="text-sm text-slate-500 mt-1">Patients will appear here when assigned via Care Team</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-xl overflow-hidden">
      {/* Header */}
      <div className="bg-linear-to-r from-slate-900 to-slate-800 px-6 py-4 border-b border-slate-700">
        ...
        <div className="flex items-center gap-3 mt-3 flex-wrap">
          {([
            { level: 'all' as const, label: `All (${counts.total})`, inactive: 'bg-slate-700 text-slate-300 hover:bg-slate-600', active: 'bg-teal-600 text-white' },
            { level: 'critical' as const, label: `Critical (${counts.critical})`, inactive: 'bg-red-900/30 text-red-300 hover:bg-red-900/50', active: 'bg-red-600 text-white' },
            { level: 'high' as const, label: `High (${counts.high})`, inactive: 'bg-orange-900/30 text-orange-300 hover:bg-orange-900/50', active: 'bg-orange-500 text-white' },
            { level: 'moderate' as const, label: `Moderate (${counts.moderate})`, inactive: 'bg-yellow-900/30 text-yellow-300 hover:bg-yellow-900/50', active: 'bg-yellow-500 text-white' },
            { level: 'low' as const, label: `Low (${counts.low})`, inactive: 'bg-green-900/30 text-green-300 hover:bg-green-900/50', active: 'bg-green-600 text-white' },
      ...
      <div className="p-4 space-y-3 max-h-[700px] overflow-y-auto">
        {filteredPatients.length === 0 ? (
          <div className="text-center py-8 text-slate-500">No patients match this filter.</div>
        ) : (
      ...
      <div className="px-6 py-3 bg-slate-900/50 border-t border-slate-700 text-xs text-slate-500">
        <strong className="text-slate-400">AI Scoring:</strong> Location (30%) + Alerts (40%) + Readmission (25%) + Fall Risk (15%) + Engagement (10%) + Vitals (10%)
...
export default NursePatientPriorityBoard;
