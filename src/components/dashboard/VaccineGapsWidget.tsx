...
const VaccineGapsWidget: React.FC = () => {
  ...
  const getPriorityColor = (monthsSinceLast: number | null) => {
    if (monthsSinceLast === null) return 'border-yellow-400 bg-yellow-50 dark:bg-slate-800';
    if (monthsSinceLast >= 24) return 'border-red-400 bg-red-50 dark:bg-red-950';
    if (monthsSinceLast >= 12) return 'border-orange-400 bg-orange-50';
    return 'border-yellow-400 bg-yellow-50 dark:bg-slate-800';
  };
    ...
    return (
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-xs border border-gray-200 dark:border-slate-700 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100">⚠️ Vaccine Care Gaps</h3>
        </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-lg shadow-xs border border-gray-200 dark:border-slate-700 p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100">⚠️ Vaccine Care Gaps</h3>
        {gaps.length > 0 && (
          <span className="px-2 py-1 bg-yellow-100 dark:bg-yellow-950 text-yellow-800 dark:text-yellow-300 text-xs font-semibold rounded-full">
            {gaps.length}
          </span>
        )}
      </div>
