...
export const CheckInHistory: React.FC<CheckInHistoryProps> = ({ history, branding }) => {
  ...
  return (
    <div className="mt-8 bg-white dark:bg-slate-900 rounded-xl shadow-md p-4 sm:p-6">
      <h3 className="text-lg font-semibold mb-4 text-center" style={{ color: branding.primaryColor || '#003865' }}>
        Your Recent Check-Ins
      </h3>
      <ul className="space-y-3 text-sm text-gray-700 dark:text-slate-300 max-h-64 overflow-y-auto">
        {history
          .slice(-LOCAL_HISTORY_CAP)
          .slice()
          .reverse()
          .map((h, i) => (
            <li key={`${h.timestamp}-${i}`} className="p-3 border-l-4 border-[#8cc63f] bg-gray-50 dark:bg-slate-800 rounded-r-lg">
              <strong className="text-gray-900 dark:text-slate-100">{h.label}</strong>
              <span className="text-gray-500 dark:text-slate-400 ml-2">&mdash; {new Date(h.timestamp).toLocaleString()}</span>
