    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-md p-8">
      ...
      <div className="space-y-3">
        {upcomingDoses.map((dose: UpcomingDose, idx: number) => (
          <div key={idx} className="flex items-center justify-between bg-gray-50 dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
            <div className="flex items-center gap-3">
              <div className="bg-purple-100 dark:bg-purple-950 rounded-full p-2">
                <Clock className="w-5 h-5 text-purple-600" />
              </div>
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-slate-100">{dose.medication_name}</h3>
                <p className="text-sm text-gray-600 dark:text-slate-400">{dose.dosage} - {dose.instructions}</p>
        ...
        {upcomingDoses.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-slate-400">No upcoming doses</p>
            <p className="text-gray-400 dark:text-slate-500 text-sm">You're all caught up!</p>
          </div>
        )}
      </div>
    </div>
