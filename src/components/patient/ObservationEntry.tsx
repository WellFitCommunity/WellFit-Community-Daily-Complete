...
const ObservationEntry: React.FC<ObservationEntryProps> = ({ userId, onSuccess, onCancel }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-lg shadow-xs border border-gray-200 dark:border-slate-700 p-6">
      <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100 mb-4">Add New Observation</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {OBSERVATION_TEMPLATES[categoryTab].map((template) => (
                ...
                >
                  <div className="font-semibold text-gray-900 dark:text-slate-100 text-sm">{template.display}</div>
        ...
        {(selectedTemplate || categoryTab === 'custom') && (
          <>
            ...
            {(categoryTab === 'vitals' || categoryTab === 'labs') && (
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Interpretation (Optional)</label>
                <select
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-slate-300 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded-md hover:bg-gray-50 dark:hover:bg-slate-800 focus:outline-hidden focus:ring-2 focus:ring-blue-500"
            disabled={loading}
          >
            Cancel
          </button>
