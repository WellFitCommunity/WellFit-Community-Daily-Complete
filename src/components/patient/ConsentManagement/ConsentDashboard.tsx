...
const ConsentDashboard: React.FC = () => {
    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <div className="bg-yellow-50 dark:bg-slate-800 border border-yellow-200 dark:border-slate-600 rounded-lg p-4">
          <p className="text-yellow-800 dark:text-yellow-300">Please log in to manage your consent settings.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
          ...
          <div className="bg-green-50 dark:bg-slate-800 rounded-lg p-4 border border-green-200 dark:border-slate-600">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-800 dark:text-green-300">{stats.activeConsents}</p>
              </div>
          ...
          <div className="bg-blue-50 dark:bg-slate-800 rounded-lg p-4 border border-blue-200 dark:border-slate-600">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-800 dark:text-blue-300">{stats.authorizedApps}</p>
              </div>
              <Smartphone className="w-8 h-8 text-blue-500" />
            </div>
          </div>

          <div className="bg-purple-50 dark:bg-slate-800 rounded-lg p-4 border border-purple-200 dark:border-slate-600">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-purple-600 font-medium">Recent Access</p>
                <p className="text-2xl font-bold text-purple-800 dark:text-purple-300">{stats.recentAccess}</p>
              </div>
      ...
      {/* Your Rights Notice */}
      <div className="mt-6 bg-blue-50 dark:bg-slate-800 border border-blue-200 dark:border-slate-600 rounded-lg p-4">
        <h3 className="font-semibold text-blue-800 dark:text-blue-300 mb-2">Your Data Rights</h3>
...
const OverviewTab: React.FC<OverviewTabProps> = ({ userId, onStatsUpdate }) => {
  ...
  if (consents.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100 mb-2">No Active Consents</h3>
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100 mb-4">Active Data Sharing</h3>
...
const ConsentCard: React.FC<ConsentCardProps> = ({ consent, onRevoke }) => {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      default:
        return <Shield className="w-5 h-5 text-gray-600 dark:text-slate-400" />;
    }
  };
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">{consent.purpose}</p>
              ...
              {consent.scopes_granted && consent.scopes_granted.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {consent.scopes_granted.slice(0, 3).map((scope: string, idx: number) => (
                    <span
                      key={idx}
                      className="text-xs bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 px-2 py-1 rounded"
                    >
...
const ProvidersTab: React.FC<ProvidersTabProps> = ({ userId }) => {
  ...
  if (providers.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100 mb-2">No Care Team Members</h3>
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100 mb-4">Your Care Team</h3>
      ...
      {providers.map((member) => (
        <div key={member.id} className="border rounded-lg p-4">
          <div className="flex items-center">
            <div className="w-10 h-10 bg-blue-100 dark:bg-blue-950 rounded-full flex items-center justify-center mr-4">
