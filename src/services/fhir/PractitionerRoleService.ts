...
export const PractitionerRoleService = {
  ...
  async getActiveByPractitioner(practitionerId: string): Promise<FHIRPractitionerRole[]> {
    const { data, error } = await supabase
      ...
      .select('id, practitioner_id, organization_id, location_id, active, code, code_display, specialty, specialty_display, period_start, period_end, telecom, available_time, not_available, endpoint_references, created_at, updated_at')
