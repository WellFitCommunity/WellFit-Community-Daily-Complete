...
export const PractitionerService = {
  ...
  async getById(id: string): Promise<FHIRPractitioner | null> {
    const { data, error } = await supabase
      ...
      .select('id, user_id, external_id, external_system, active, npi, state_license_number, dea_number, taxonomy_code, family_name, given_names, prefix, suffix, gender, birth_date, email, phone, telecom, addresses, photo_url, qualifications, specialties, specialty_codes, communication_languages, bio, availability_hours, created_by, updated_by, created_at, updated_at')
      .eq('id', id)
      .single();
