...
CREATE INDEX idx_chw_kiosk_sessions_patient ON public.chw_kiosk_sessions(patient_id);
CREATE INDEX idx_chw_kiosk_sessions_kiosk ON public.chw_kiosk_sessions(kiosk_id);
CREATE INDEX idx_chw_kiosk_sessions_check_in ON public.chw_kiosk_sessions(check_in_time);
CREATE INDEX idx_chw_kiosk_sessions_location ON public.chw_kiosk_sessions USING GIST(location_gps);
CREATE INDEX idx_chw_kiosk_sessions_visit ON public.chw_kiosk_sessions(visit_id);
CREATE INDEX idx_chw_kiosk_sessions_data ON public.chw_kiosk_sessions USING GIN(session_data);
...
CREATE INDEX idx_chw_kiosk_devices_location ON public.chw_kiosk_devices USING GIST(location_gps);
CREATE INDEX idx_chw_kiosk_devices_service_area ON public.chw_kiosk_devices USING GIST(service_area);
CREATE INDEX idx_chw_kiosk_devices_active ON public.chw_kiosk_devices(is_active);
CREATE INDEX idx_chw_kiosk_devices_type ON public.chw_kiosk_devices(location_type);
...
CREATE INDEX idx_chw_kiosk_analytics_kiosk ON public.chw_kiosk_usage_analytics(kiosk_id);
CREATE INDEX idx_chw_kiosk_analytics_date ON public.chw_kiosk_usage_analytics(date);
...
CREATE POLICY "CHW staff can view all kiosk sessions"
  ...
  USING (
    EXISTS (
      ...
      WHERE id = auth.uid() AND role IN ('admin', 'super_admin', 'nurse', 'case_manager')
    )
  );
...
CREATE POLICY "Staff can view all patient consents"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = auth.uid() AND role IN ('admin', 'super_admin', 'physician', 'nurse', 'case_manager')
    )
  );
...
-- Migration complete
COMMENT ON SCHEMA public IS 'CHW Kiosk System migration completed successfully';
