-- Restore: Encounter Provider Assignment System
--
-- DRIFT FORENSIC (DB-reference drift triage #2 + #35 — rpc::assign_encounter_provider,
-- rpc::remove_encounter_provider):
--   The original migration 20260212000001_encounter_provider_assignment.sql is
--   registered as APPLIED in schema_migrations, yet NONE of its objects exist in
--   the live DB (no encounter_providers / encounter_provider_audit tables, no
--   assign/remove/validate functions, no encounter_provider_role enum).
--   Root cause: that file carries a `-- migrate:down` block (dbmate/golang-migrate
--   syntax) AFTER the up-section COMMIT. Supabase CLI (`db push`) does NOT split on
--   `-- migrate:up`/`-- migrate:down` markers — it runs the WHOLE file. So the
--   migration created every object and then immediately DROPped it all in the same
--   run. Net effect: nothing, but the registry recorded success.
--   The feature is REACHABLE (admin UI: EncounterProviderPanel.tsx,
--   ProviderAssignmentDashboard.tsx + the CPOE order forms' useOrderingProvider),
--   so the live objects are genuinely missing, not dead code.
--
--   This migration re-applies ONLY the up-section (no migrate:down block). All
--   dependencies were verified present live before authoring: encounters,
--   billing_providers, is_admin(uuid), set_updated_at(), and the sibling
--   state-machine objects (encounter_valid_transitions, encounter_status_history,
--   encounters.status/status_changed_at) — that sibling did NOT self-destruct.
--   Idempotent (IF NOT EXISTS / CREATE OR REPLACE / duplicate_object guard).
--
BEGIN;
