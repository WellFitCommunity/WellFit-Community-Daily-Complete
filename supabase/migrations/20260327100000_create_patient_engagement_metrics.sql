-- Create patient_engagement_metrics table + supporting objects
-- These were defined in 20251123000000 but never applied to live DB
-- Referenced by: engagementFactors.ts, communicationSilenceWindowService.ts, metrics.ts

-- 1. Create patient_engagement_metrics table
...
CREATE INDEX IF NOT EXISTS idx_engagement_metrics_score
  ...
  WHERE overall_engagement_score < 40;
...
-- Grants
GRANT SELECT, INSERT, UPDATE ON public.patient_engagement_metrics TO authenticated;
GRANT SELECT ON public.patient_engagement_metrics TO anon;
GRANT ALL ON public.patient_engagement_metrics TO service_role;
...
-- 2. Add feature columns to readmission_risk_predictions (if missing)
DO $$
BEGIN
...
-- 3. Indexes for feature-based queries
...
-- 4. View for high-risk patients with feature breakdown
CREATE OR REPLACE VIEW public.v_high_risk_readmission_patients
...
-- 5. Engagement warning score function
