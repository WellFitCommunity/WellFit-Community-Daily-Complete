...
-- Indexes
CREATE INDEX idx_drills_type ON disaster_recovery_drills(drill_type);
CREATE INDEX idx_drills_status ON disaster_recovery_drills(status);
CREATE INDEX idx_drills_scheduled_start ON disaster_recovery_drills(scheduled_start DESC);
CREATE INDEX idx_drills_scenario ON disaster_recovery_drills(drill_scenario);
...
CREATE INDEX idx_checkpoints_drill_id ON drill_checkpoints(drill_id);
CREATE INDEX idx_checkpoints_order ON drill_checkpoints(drill_id, checkpoint_order);
...
CREATE INDEX idx_participants_drill ON drill_participants(drill_id);
CREATE INDEX idx_participants_user ON drill_participants(user_id);
...
CREATE INDEX idx_metrics_drill ON drill_metrics_log(drill_id);
CREATE INDEX idx_metrics_category ON drill_metrics_log(metric_category);
...
-- Schedule initial drills
DO $$
DECLARE
  v_next_sunday TIMESTAMP WITH TIME ZONE;
  v_next_month TIMESTAMP WITH TIME ZONE;
BEGIN
  -- Next Sunday at 2 AM UTC
  v_next_sunday := DATE_TRUNC('week', NOW() + INTERVAL '1 week') + INTERVAL '2 hours';

  -- First Saturday of next month at 10 AM
  v_next_month := DATE_TRUNC('month', NOW() + INTERVAL '1 month') + INTERVAL '5 days 10 hours';

  -- Schedule weekly drill
  PERFORM schedule_disaster_recovery_drill(
    'Weekly Automated Drill - ' || TO_CHAR(v_next_sunday, 'YYYY-MM-DD'),
    'weekly_automated',
    'database_loss',
    v_next_sunday
  );
  ...
  PERFORM schedule_disaster_recovery_drill(
    'Monthly Simulation Drill - ' || TO_CHAR(v_next_month, 'YYYY-MM'),
    'monthly_simulation',
    'security_breach',
    v_next_month
  );
