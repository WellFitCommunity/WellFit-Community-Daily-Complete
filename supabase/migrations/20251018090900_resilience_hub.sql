...
-- ============================================================================
-- PART 6: WORKLOAD ANALYTICS (SKIPPED - REQUIRES 'encounters' TABLE)
-- ============================================================================
-- NOTE: This section is commented out because it depends on the 'encounters' table
-- which doesn't exist in the current database. Uncomment and run as a separate
-- migration after the encounters table is created.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_workload_metrics_practitioner ON provider_workload_metrics(practitioner_id);
...
-- Placeholder notice for when encounters table exists
DO $$
BEGIN
  RAISE NOTICE '⚠️  Workload Analytics (PART 6) is currently disabled because the "encounters" table does not exist.';
  RAISE NOTICE '   To enable workload analytics:';
  RAISE NOTICE '   1. Ensure encounters table exists';
  RAISE NOTICE '   2. Uncomment PART 6 in this migration file';
  RAISE NOTICE '   3. Run as a separate migration';
END $$;
...
-- COMMENT ON MATERIALIZED VIEW provider_workload_metrics IS 'Aggregated workload data from encounters and check-ins. Refresh nightly or on-demand.';  -- Commented out because view doesn't exist yet
