...
**Last Updated:** 2026-07-10 (third session)

---
### 📨 HANDOFF FOR NEXT SESSION (read this first)
**This session's arc:** Picked up the two MCP follow-ups the *second* session left (the `.select('*')` §9 pass + the "does this column exist live?" audit). The audit blew up: **most of the MCP FHIR server was runtime-dead** — column lists written to the FHIR-R4 spec that never matched the actual local schema. Maria said "Fix all now, one commit per resource." **13 commits, all on `main`+pushed, all live-verified & deployed; nothing mid-flight.**
- **`export_patient_bundle` was FULLY DEAD** (selected `date_of_birth`/`middle_name`/`address_line1` from `profiles`; live cols are `dob` + single `address`, no middle_name) → threw "Patient not found" for every patient. Fixed across 6 files; the junk Patient tests were rewritten to actually call `toFHIRPatient`. Proven: exact query returns a live patient row.
- **A real SECURITY bug caught by the audit:** `mcp-chain-orchestrator` approve-step looked up `approval_role` from table **`chain_steps` (doesn't exist)** → query errored → `requiredRole` undefined → **approval-role gate silently skipped** (wrong-role user could approve). Repointed to `chain_step_definitions` (`72d2b411`, live-proven `to_regclass('chain_steps') IS NULL`).
- **7 more FHIR resources had `42703` column drift** (DiagnosticReport, Practitioner, Encounter, Medication, Procedure, Immunization, Goal) + `get_patient_summary` broken in nearly every section. One commit per resource. **0 of 33 corrected columns missing live.**
- **AllergyIntolerance + DocumentReference were aspirational table names** (`fhir_allergies`/`fhir_document_references` never existed). Backed both with `security_invoker` **VIEWS** (governance boundary-layer pattern) over the real `allergy_intolerances` (patient_id←user_id) and `ai_progress_notes` — **zero code change** since `FHIR_TABLES` already named them. Migrations `20260710092103`/`092104`, applied via `db push --include-all`, live-verified (views exist, `security_invoker=on`, GRANT true). `ai_risk_assessments`→`risk_assessments` repoint.
- **🛡️ ROOT CAUSE RETIRED — the FHIR schema-drift gate now covers the MCP server.** Extended `scripts/check-fhir-service-schema.py` (already wired in `ci-cd.yml:87`, runs every push) to (a) scan `supabase/functions/mcp-fhir-server/*.ts` not just `src/services/fhir/`, (b) parse table-keyed column maps like `FHIR_SELECT_COLUMNS` (the map values were the drift source, and it only looked at `.select()` calls before), (c) validate ANY snapshot table (profiles + the new views), (d) treat dynamic `.from(variable)` as a barrier (killed 5 false positives). Snapshot refreshed from live 27→36 tables (`fhir-schema-snapshot.json`; refresh SQL updated). **On this session's pre-fix tree the extended gate flagged 3 MORE real runtime-dead paths the resource pass missed** — `get_care_team` selecting `fhir_practitioners.name/specialty` + `fhir_connections` `ehr_type/base_url/sync_mode/last_sync_at` in 3 handlers (list/trigger/map) — all fixed + deployed (`d15b89c2`), gate now green (38 files, 0 findings). Self-tested: injecting a bogus column into the map → gate fails naming it → revert → green. Commits `d15b89c2` (fixes) + `773d9db3` (gate). **Refresh the snapshot after any migration touching a covered table** (`scripts/refresh-fhir-schema-snapshot.sql` → paste into the json). Generalizing this to the other mcp-* servers (chain/coding/drg — same drift class) is the natural next extension.
- **⚠️ OPEN FOLLOW-UPS for you (surfaced, NOT done):**
  1. **Real-data round-trips: ✅ AllergyIntolerance + DocumentReference PROVEN; ❌ RiskAssessment BLOCKED.** Seeded synthetic rows for test patient **Demo Senior** (`ad402b9b…`, `is_test_user=true`) and round-tripped through the **deployed** MCP tools: `mcp__fhir__search_resources`(AllergyIntolerance) → `Bundle total:1`; `get_patient_summary`(allergies) populated; `search_resources`(DocumentReference) → `Bundle total:1` (via the `ai_progress_notes` view). Synthetic rows deleted after (0 left). **RiskAssessment could NOT be seeded → NEW FINDING (see below).**
  1b. **🔴 NEW FINDING — `risk_assessments` is UN-INSERTABLE live.** Its `BEFORE INSERT` trigger `encrypt_risk_assessments_phi()` calls `public.encrypt_phi_jsonb(jsonb)` **which does not exist** (only `encrypt_phi_text` exists) → every insert errors `42883`. So the risk-assessment **write** feature is dead in prod, and the FHIR RiskAssessment read path (`export_patient_bundle` include_ai, repointed in `96105a51`) is column-verified but can't be real-data-proven until this is fixed. **PHI-encryption territory (supabase.md §17, two-key footgun) → Akima/Maria before touching.** **INVESTIGATED + written up:** root cause = `encrypt_phi_jsonb` was dropped by `20251209110000_drop_broken_functions.sql` (l.367) but the trigger `trg_ra_encrypt`/`encrypt_risk_assessments_phi()` still calls it; the trigger is also **untracked** (no repo migration defines it); `encrypt_phi_text(data, use_clinical_key DEFAULT false)` → trigger uses the **WellFit** key on a **clinical** table (§17 mismatch); **no `risk_assessments_decrypted` view exists** live; and the trigger NULLs plaintext after encrypting, so the FHIR `basis` (reads plaintext `risk_factors`) would be empty. Full proposal (recommend **Option A**: switch the trigger to the surviving `encrypt_phi_text`, don't revive the dropped fn) in **`docs/clinical/RISK_ASSESSMENTS_ENCRYPTION_REVIEW.md`** (commit `a46c4deb`; named `*_REVIEW.md` because `.gitignore` excludes `*_FIX_*.md`). **Akima notified** via a Gmail **draft** (unsent, in Maria's drafts — To akima@, CC maria@). **BLOCKED awaiting Akima's 4 decisions:** key scope (WellFit vs Atlus-clinical), fix option, decrypt-view scope, whether FHIR should surface decrypted risk factors. Fix (migration + decrypt view + live round-trip proof) authorable once she decides.
  2. **DocumentReference only surfaces `ai_progress_notes`** — `clinical_notes` is encounter-scoped (no `patient_id`); a future migration can UNION it via an encounter→patient join (noted in `20260710092104`'s comment).
  3. **`getPatientBundle` filters `profiles.id`, `get_patient_summary` filters `user_id`** — harmless today (`id==user_id` for all 61 rows) but flagged; if the 1:1 ever breaks this diverges.
  4. **Generic FHIR search optional filters** (`status`/`category`/`code`) may `42703` on the allergy view (it has `clinical_status`, not bare `status`) — pre-existing generic-path assumption, only bites if a caller passes those filters; the patient-scoped bundle path is fine.
  5. **`pg_constraint` name-collision sweep** (the `_check1` duplicate-constraint class from the 07-10 first session) still not done fleet-wide.
- **Deploy note:** all four touched servers redeployed via `--use-api` (verify_jwt pins preserved): mcp-chain-orchestrator, mcp-medical-coding-server, mcp-drg-grouper-server, mcp-fhir-server. deno lives at `~/.deno/bin/deno` (NOT on PATH).
---
