...
**Last Updated:** 2026-05-29 (fourth session)
**Last Session:** **Closed a live super_admin self-escalation hole, finished ONC Tier 2 (ONC-8 + ONC-9), and decomposed/repaired bulk-export (ONC-10 bulk path done).** (1) Security: the `profiles` UPDATE guard never checked `role_id` (the RLS-enforced column) — any user could `SET role_id` and self-promote. Hardened the trigger + added `admin_assign_role` RPC, wired `userRoleManagementService.assignRole` to it (the old direct UPDATE silently no-op'd under own-row RLS), fixed a phantom `profiles.department` ref, added `profiles.department`, and built a GUC trusted-path handshake after a `current_user` bypass attempt briefly re-opened the hole (caught by a live rolled-back test). `01eb0b87`/`9db8fc87`/`519ebe42`/`6f3dff1e`. (2) Reconciled MCP migration-registry drift + added a PreToolUse hook blocking `apply_migration` via MCP. (3) **ONC-8** (a)(12) FamilyMemberHistory — full ONC-5-shaped feature. `72d4e0ed`. **VISUAL ACCEPTED by Maria 2026-05-29.** (4) **ONC-9** (d)(6) break-the-glass — `BreakTheGlassModal` + chart entry + grant/revoke + new `notify-emergency-access` edge fn (deployed). `1f4d50c9`/`92f135e6`. **VISUAL ACCEPTED by Maria 2026-05-29.** (5) **bulk-export decomposed** (868→6 modules <600) + repaired its export_jobs schema drift (it wrote 6 nonexistent columns → was non-functional) + ONC-10 SHA-256 integrity wired + dynamic SELECT-column resolution (`get_exportable_columns` RPC + `_shared/exportColumns.ts`). `fb8642c7`/`ad1d4c0a`.
**Updated By:** Claude Opus 4.8 (1M context)
**Codebase Health:** ~11,910+ tests passing (this session added: 14 family-history + 8 break-the-glass + 4 role-RPC + 1 notify ≈ 27 net new; bulk-export change is edge-fn Deno, verified via `deno check` + live rolled-back proof, not vitest). 0 lint warnings, 0 typecheck errors (scoped). Migration registry in sync (`db push --dry-run` → up to date). 6 migrations applied this session (role-escalation, department+trusted-path, GUC fix, FamilyMemberHistory, export_jobs columns + get_exportable_columns RPC). 2 edge fns deployed (notify-emergency-access, bulk-export). FamilyHistory + BreakTheGlass UIs **visually accepted by Maria 2026-05-29**.
...
> **🎯 NEXT SESSION STARTS HERE: decompose + repair `ccda-export/index.ts` (836 lines), then wire ONC-10 integrity into it.** This is the LAST ONC Tier 2 item. WARNING from this session: bulk-export had hidden schema drift (it wrote 6 columns that didn't exist on `export_jobs` → non-functional). **Do NOT blind-decompose ccda-export — first verify every table/column it reads & writes against the live DB (Rule #18).** Follow the bulk-export pattern: extract modules <600, use `_shared/exportColumns.ts` + `get_exportable_columns` for SELECT columns (no bare `*`), compute SHA-256 via `_shared/integrityHash.ts`, `deno check` with `~/.deno/bin/deno`, deploy, live rolled-back proof. After ccda-export, ONC Tier 2 is fully closed → move to Tier 3 (ONC-11 WCAG, ONC-13 matrix) or Guardian Session 2.

### ONC Certification — Tier 1 ✅ 5/5 + Tier 2 ✅ 6/6 buildable done (ONC-6/7/8/9 + ONC-10 FHIR & bulk paths); only ONC-10 **ccda** wiring remains (next session)
...
- ⬜ **ONC-10 ccda path — NEXT SESSION.** `ccda-export/index.ts` (836 lines) — same treatment as bulk-export. **VERIFY ITS LIVE SCHEMA FIRST** (bulk-export taught us these have hidden drift). This is the final ONC Tier 2 item.
