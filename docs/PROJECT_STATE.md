...
**Codebase Health:** All scoped checks green per commit — scoped typecheck 0, full `tsc` clean on touched files, lint 0/0; touched suites green (fhirSecurity 76, encounter-provider 28, ProcedureService 17, fhirQuestionnaire 37, QuestionnaireStatsPanel 4). **5 migrations applied via `db push` + live-proven; snapshot refreshed (757 tables / 1454 functions). No edge-fn deploys, no auth changes.** `rpc::` drift baseline **36 → 27**. ⏳ **Awaiting Maria:** visual acceptance of the new QuestionnaireStatsPanel + one in-app admin click on the FHIR "🚀 Deploy to WellFit" button (RLS-gated write can't be driven from the service-role console).
...
> **▶ NEXT SESSION STARTS HERE (updated 2026-06-04): continue the `rpc::` drift triage — 27 left of 36.** Tracker `docs/trackers/db-reference-drift-triage-tracker.md` is the airtight spec (per-entry buckets + the VERDICT MAP for the 15 broken-dropped + two SYSTEMIC FINDINGS). **Easiest next (deps present live, no schema rebuild):** `get_expiring_consents` → repoint to live `patient_consents`; `get_vulnerability_summary` → repoint to live `security_vulnerabilities`. **Then the schema-rebuild clusters** (Maria authorized rebuilding properly): time-clock (`get_team_time_entries`/`get_tenant_time_entries` → `clinician_time_tracking`/`time_entries` MISSING), behavioral (`get_uninvestigated_anomalies` → `behavioral_anomalies` MISSING), law-enforcement (`get_missed_check_in_alerts`/`get_welfare_check_info` → welfare/missed-checkin tables MISSING — sensitive). **Maria sign-off items:** clearinghouse pair (Vault secrets), `calculate_readmission_risk_score` (clinical algo). **Likely C-dead (confirm w/ Maria):** `auto_generate_clinical_data_for_hospital_patient` (demo seeder). The 156 `table::` entries come after the rpc half. **Per-entry recipe (with the migrate:down forensic) is in the tracker.**
>
