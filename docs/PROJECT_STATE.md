...
- **⚠️ OPEN:**
  1. **CARRIED FROM THIRD SESSION — still BLOCKED on Akima:** `risk_assessments` UN-INSERTABLE live (its trigger calls the dropped `encrypt_phi_jsonb`) → PHI §17 territory. Proposal in `docs/clinical/RISK_ASSESSMENTS_ENCRYPTION_REVIEW.md`; unsent Gmail draft to Akima. Awaiting her 4 decisions. **(This is the only open item — everything else from the 2026-07-10 fourth session is done + live-verified.)**
---
