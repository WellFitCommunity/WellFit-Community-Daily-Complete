...
**Last Session:** **Closed a live super_admin self-escalation hole, then finished ONC Tier 2 (ONC-8 + ONC-9).** (1) Security: the `profiles` UPDATE guard never checked `role_id` (the RLS-enforced column) — any user could `SET role_id` and self-promote. Hardened the trigger + added `admin_assign_role` RPC, wired `userRoleManagementService.assignRole` to it (the old direct UPDATE silently no-op'd under own-row RLS), fixed a phantom `profiles.department` ref, added `profiles.department`, and built a GUC trusted-path handshake after a `current_user` bypass attempt briefly re-opened the hole (caught by a live rolled-back test). Commits `01eb0b87`/`9db8fc87`/`519ebe42`/`6f3dff1e`. (2) Reconciled MCP migration-registry drift + added a PreToolUse hook blocking `apply_migration` via MCP (it timestamps the registry off the filename → drift). (3) **ONC-8** (a)(12) FamilyMemberHistory — full mirror of ONC-5 (parent+child tables, 2 services, panel/form/list, page, route, chart nav card; 14 tests; live FK+CASCADE round-trip). `72d4e0ed`. (4) **ONC-9** (d)(6) break-the-glass — `BreakTheGlassModal` + chart entry + grant/revoke + new `notify-emergency-access` edge fn (supervisor email, deployed). `1f4d50c9`/`92f135e6`.
**Updated By:** Claude Opus 4.8 (1M context)
**Codebase Health:** ~11,910+ tests passing (this session added: 14 family-history + 8 break-the-glass + 4 role-RPC + 1 notify = ~27 net new). 0 lint warnings, 0 typecheck errors (scoped). Migration registry in sync (`db push --dry-run` → up to date). 5 migrations applied this session (role-escalation fix, department+trusted-path, GUC fix, FamilyMemberHistory). 1 edge fn deployed (notify-emergency-access). **VISUAL ACCEPTANCE PENDING (Commandment #13):** the FamilyHistory panel/form and the BreakTheGlassModal are new UI not yet seen rendered — Maria must eyeball them in the browser before they're "visually done."
...
### ONC Certification — Tier 1 ✅ 5/5 + Tier 2 ✅ buildable done (ONC-6/7/8/9 + ONC-10 FHIR path; only ONC-10 bulk/ccda wiring remains, blocked on god-file decomp)
...
- ✅ **ONC-8** (a)(12) FHIR FamilyMemberHistory — DONE (`72d4e0ed`). Parent `fhir_family_member_history` + child `fhir_family_member_history_conditions` (CASCADE), `FamilyMemberHistoryService` + `FamilyMemberHistoryConditionService`, decomposed `FamilyHistoryPanel` (orchestrator + AddFamilyHistoryForm + FamilyHistoryListView) + page + route `/admin/family-history/:patientId` + chart nav card. 14 tests. Live FK+CASCADE round-trip verified. **Visual acceptance pending.**
- ✅ **ONC-9** (d)(6) Break-the-glass — DONE (`1f4d50c9` + `92f135e6`). `BreakTheGlassModal` (reason+duration, grant/revoke) on the patient-chart Overview, on top of the DB foundation from `4b9ae432`. Supervisor notification via new `notify-emergency-access` edge fn (deployed; accessor-gated, resolves tenant admins server-side, send-email, flips supervisors_notified). 8 modal + 5 service tests; live grant+revoke round-trip verified. **Visual acceptance pending.**
