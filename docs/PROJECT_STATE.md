...
- **⚠️ OPEN FOLLOW-UPS for you (surfaced, NOT done):**
  1. **Real-data round-trips still unproven** — `allergy_intolerances` / `ai_progress_notes` / `risk_assessments` are **empty live**, so AllergyIntolerance/DocumentReference/RiskAssessment paths are proven only by column-resolution + view existence, not a seeded end-to-end call.
