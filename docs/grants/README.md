# Grant Applications & Funding Documentation

## WellFit Community, LLC

**Last Updated:** January 18, 2026

---

# QUICK START

## Immediate Priorities

| Priority | Opportunity | Deadline | Action Now |
|----------|-------------|----------|------------|
| **1** | CMS ACCESS Model | **April 1, 2026** | Register on CMS Portal |
| **2** | ~~HRSA Tech-enabled Learning~~ | ~~Feb 2, 2026~~ | **MISSED** - deadline passed |
| **3** | USDA DLT Grant | **Spring 2026** | Find rural partner |
| **4** | HRSA Chronic Care Telehealth | **April 24, 2026** | Find academic partner |
| **5** | NIH PAR-25-170 | **June 22, 2026** | Finalize draft |

---

# DOCUMENT INDEX

## Grant Applications (Ready to Submit)

| Document | Program | Amount | Deadline | Status |
|----------|---------|--------|----------|--------|
| [CMS_ACCESS_MODEL_APPLICATION.md](CMS_ACCESS_MODEL_APPLICATION.md) | CMS ACCESS Model | $990K/yr (ongoing) | April 1, 2026 | Complete |
| [NIH_PAR-25-170_GRANT_APPLICATION.md](NIH_PAR-25-170_GRANT_APPLICATION.md) | NIH Digital Biomarkers | $3M/5 years | June 22, 2026 | Complete |
| [AHRQ_R21_R33_APPLICATION.md](AHRQ_R21_R33_APPLICATION.md) | AHRQ Digital Healthcare | $1M/5 years | Rolling | Complete (needs academic partner) |
| [USDA_DLT_GRANT_APPLICATION.md](USDA_DLT_GRANT_APPLICATION.md) | USDA Telehealth | $500K | Spring 2026 | Template (needs partner) |

## Executive Summaries

| Document | Description |
|----------|-------------|
| [CMS_ACCESS_MODEL_EXECUTIVE_SUMMARY.md](CMS_ACCESS_MODEL_EXECUTIVE_SUMMARY.md) | 2-page CMS ACCESS summary |
| [NIH_PAR-25-170_EXECUTIVE_SUMMARY.md](NIH_PAR-25-170_EXECUTIVE_SUMMARY.md) | 2-page NIH summary with checklist |
| [AHRQ_R21_R33_EXECUTIVE_SUMMARY.md](AHRQ_R21_R33_EXECUTIVE_SUMMARY.md) | 1-page AHRQ summary with requirements |

## Reference Documents

| Document | Description |
|----------|-------------|
| [2026_FUNDING_OPPORTUNITIES.md](2026_FUNDING_OPPORTUNITIES.md) | All 2026 funding opportunities overview |
| [ENVISION_ATLUS_GRANTS.md](ENVISION_ATLUS_GRANTS.md) | Clinical AI, CDS, and interoperability grants |
| [NIH_PAR-25-170_CAPABILITY_MAPPING.md](NIH_PAR-25-170_CAPABILITY_MAPPING.md) | Platform capabilities → grant requirements |
| [RURAL_HEALTH_PARTNERSHIP_GRANTS.md](RURAL_HEALTH_PARTNERSHIP_GRANTS.md) | Rural partnership grant overview |

## Outreach & Partnership Development

| Document | Description |
|----------|-------------|
| [LETTERS_OF_SUPPORT.md](LETTERS_OF_SUPPORT.md) | 20 letter templates (all partner types) |
| [RURAL_PARTNER_OUTREACH.md](RURAL_PARTNER_OUTREACH.md) | Outreach letters, emails, follow-up sequences |
| [PROSPECT_TRACKING.md](PROSPECT_TRACKING.md) | Pipeline tracking template |
| [TEXAS_PROSPECT_LIST.md](TEXAS_PROSPECT_LIST.md) | Texas-specific prospects and contacts |
| [AHRQ_PILOT_PROSPECTS.md](AHRQ_PILOT_PROSPECTS.md) | Academic partners & pilot sites for AHRQ R21/R33 |

---

# FUNDING OPPORTUNITIES SUMMARY

## Federal Grants

### WellFit + Community Health Grants

| Program | Agency | Amount | Deadline | Application |
|---------|--------|--------|----------|-------------|
| ACCESS Model | CMS/CMMI | Ongoing payments | April 1, 2026 | Complete |
| PAR-25-170 | NIH | $3M/5 years | June 22, 2026 | Complete |
| USDA DLT | USDA | $50K-$1M | Spring 2026 | Template ready |
| HRSA Network Dev | HRSA | $900K/3 years | TBD 2026 | Outline ready |
| HRSA Telehealth | HRSA | $1M/4 years | TBD 2026 | Outline ready |
| RCORP-Impact | HRSA | $4M/4 years | April 2026 | Not started |
| FCC Healthcare Connect | FCC | 65% discount | Rolling | Not started |

### Envision Atlus-Specific Grants (Clinical AI/CDS)

| Program | Agency | Amount | Deadline | Application |
|---------|--------|--------|----------|-------------|
| AI Healthcare Safety (R18) | AHRQ | Up to $1.5M | Rolling | Not started |
| Digital Healthcare (R21/R33) | AHRQ | $1M/5 years | Rolling | **Complete** |
| ONC LEAP | ONC | $200K-$500K | Spring 2026 | Not started |
| Chronic Care Telehealth | HRSA | TBD (large) | April 24, 2026 | Not started |
| ~~Tech-enabled Learning~~ | ~~HRSA~~ | ~~$475K~~ | ~~Feb 2, 2026~~ | **Missed deadline** |
| CMS QIO Partnership | CMS | Vendor contract | Ongoing | Not started |

## Estimated Total Potential Funding

| Timeframe | Grants | Potential Funding |
|-----------|--------|-------------------|
| 2026 | ACCESS + USDA DLT | $500K + ongoing |
| 2026-2031 | NIH PAR-25-170 | $3,000,000 |
| Ongoing | ACCESS Model payments | $990K-$9.9M/year |
| 2026-2028 | AHRQ AI/CDS (R21/R33) | $275K-$2M |
| 2026-2030 | ONC LEAP | $200K-$500K |
| **TOTAL** | **All Programs** | **$5M-$15M+** |

---

# KEY CONTACTS

## Texas State Office of Rural Health

| Resource | Contact |
|----------|---------|
| SORH General | RuralHealth@TexasAgriculture.gov / (512) 936-6730 |
| TORCH (Texas Rural Hospitals) | [torchnet.org](https://www.torchnet.org) / (512) 873-0045 |

## Federal Program Contacts

| Program | Contact |
|---------|---------|
| CMS ACCESS Model | ACCESSModelTeam@cms.hhs.gov |
| NIH PAR-25-170 | See NOFO for program officer contacts |
| USDA DLT | See NOFO for regional contacts |

---

# TIMELINE

## January 2026
- [x] Draft CMS ACCESS Model application
- [x] Draft NIH PAR-25-170 application
- [x] Create letters of support templates
- [x] Create rural outreach materials
- [ ] Contact Texas SORH
- [ ] Contact TORCH

## February 2026
- ~~[ ] HRSA Tech-enabled Learning~~ - **Missed (Feb 2 deadline)**
- [ ] Discovery calls with rural partners
- [ ] CMS ACCESS application refinement
- [ ] Begin Medicare Part B enrollment (if needed)
- [ ] Recruit Clinical Director

## March 2026
- [ ] Finalize ACCESS application
- [ ] Execute MOU with rural partner (USDA DLT)
- [ ] CMS API integration development
- [ ] Obtain letters of support

## April 2026
- [ ] **April 1: Submit CMS ACCESS Model application**
- [ ] Finalize USDA DLT application with partner
- [ ] NIH application refinement

## May 2026
- [ ] Submit USDA DLT application (expected deadline)
- [ ] NIH application final review

## June 2026
- [ ] **June 22: Submit NIH PAR-25-170 application**

## July 2026
- [ ] **July 5: CMS ACCESS Model starts (if awarded)**

---

# CHECKLIST: GRANT SUBMISSION REQUIREMENTS

## CMS ACCESS Model

- [ ] Register on CMS Participant Portal
- [ ] Complete Request for Applications form
- [ ] Medicare Part B enrollment confirmed
- [ ] Clinical Director designated (MD/DO)
- [ ] HIPAA compliance documentation
- [ ] Technology capability documentation
- [ ] Implementation plan
- [ ] Quality measurement framework

## NIH PAR-25-170

- [ ] SF424 (R&R) forms
- [ ] Specific Aims (1 page)
- [ ] Research Strategy (12 pages)
- [ ] Budget and justification
- [ ] Biographical sketches (all key personnel)
- [ ] Letters of support (patient advocacy orgs required)
- [ ] Human subjects documentation
- [ ] Data sharing plan

## USDA DLT

- [ ] Rural partner identified
- [ ] Lead applicant registered in SAM.gov
- [ ] Lead applicant registered in Grants.gov
- [ ] 15% match documented
- [ ] Rural area documentation (RUCA codes)
- [ ] Budget and justification
- [ ] Technology specifications
- [ ] Letters of commitment

---

# NOTES

## Grant Writing Tips

1. **Start with the reviewer** - What do they need to say "yes"?
2. **Be specific** - Numbers, names, dates, not vague promises
3. **Show capability** - Platform is production-ready, not vaporware
4. **Address sustainability** - How does this continue after grant?
5. **Health equity matters** - Rural, underserved, diverse populations

## Partnership Development Tips

1. **Lead with value** - What's in it for them?
2. **Minimize their burden** - We do the work
3. **Be patient** - Rural health leaders are busy
4. **Use intermediaries** - SORH and associations open doors
5. **Build relationships** - Even if this grant doesn't work, next one might

---

# VERSION HISTORY

| Date | Version | Changes |
|------|---------|---------|
| 2026-01-18 | 1.0 | Initial creation of all grant documents |

---

*Grant Documentation Index | WellFit Community, LLC*
*For questions: maria@wellfitcommunity.com*
