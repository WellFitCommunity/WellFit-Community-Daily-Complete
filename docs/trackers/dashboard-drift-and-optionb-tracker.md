...
- [x] **C4 `realtime_medical_transcription` 502** — ✅ FIXED (`870e11be`). Root cause: `verify_jwt = true` on a **WebSocket** function. Browsers can't set an Authorization header on a WS upgrade (token rides in `?access_token=`), so gateway JWT verification failed the handshake → 502 with `function_id: null` (never reached the function). The function already auth-verifies internally (`admin.auth.getUser(access_token)`, index.ts L107), so gateway verify was redundant + blocking. Set `verify_jwt = false`, redeployed (bundled clean 185.6kB → no boot error). **Live-proven: 502 → 426** (raw WS probe now hits the function's own L95 `Expected WebSocket`, i.e. past the gateway). Real browser WS → 101 is the final in-app visual sign-off.
