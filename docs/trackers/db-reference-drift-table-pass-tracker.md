...
## Progress
- **2026-06-11 (eighth) — drift gate is now EMBED-AWARE; surfaced 2 pre-existing embed drifts.** `check-db-reference-drift.py` previously matched only `.from()`/`.rpc()` — PostgREST resource embeds (`patient:patients(...)`, `patients!inner(...)`, `code_icd(desc)`) inside `.select()` strings were invisible (this is what let the 5 encounter embed sister-bugs survive). Added `RE_SELECT`+`RE_EMBED`: scans select strings, strips `alias:` prefixes and `!fk`/`!inner` hints, skips aggregates (`count()`) and FK-column embeds (`alias:origin_unit_id(...)` — names ending `_id`/`_by`/`_to` are FK columns, not tables). It immediately found **2 real pre-existing embed drifts** (baselined for triage, 151→153): (1) **`table::patients`** — `immunization-registry-submit:83` `patients!inner(...)`, an embed my manual `:patients(` grep MISSED (bare `!inner` form, no colon) — but its base table `immunizations` is itself already-drifted (doesn't exist live), so this clears only when the immunization-registry drift is resolved; (2) **`table::code_icd`** — `encounterBillingBridgeService:346` `code_icd(desc)` → should be `code_icd10`, but `encounter_diagnoses` has NO FK to it and the column is `description` not `desc` → needs a reference-data FK decision (Maria). **Net: the `.from('patients')` callers are all still done; the gate now also guards embed drift going forward.**
- **2026-06-11 (seventh) — A2 `patients` DONE ✅ — baseline entry removed (141→140).** encounterService resolved via Maria's chosen path (**add a real FK, embed cleanly**): discovered `encounters.patient_id` already had a FK to **auth.users** (name collision made my first migration a no-op — `information_schema` had hidden it because the target is in the `auth` schema). Added a distinctly-named FK `encounters_patient_id_profiles_fkey → profiles(id)` (`20260611120100`, via `db push`; 0 orphans/0 nulls pre-verified; profiles.id is UNIQUE). Repointed all 3 encounter-read embeds + the 4 dead CRUD methods (→ profiles, `user_id`-keyed, ssn/member_id dropped per SS-5, address/zip aliased). **5 MORE embed SISTER BUGS** found via `:patients(` grep (a syntax the `.from()` gate never tracked) — all `.from('encounters')` embeds in `sdohBillingService` (×2), `generate-837p`, `sdoh-coding-suggest` — repointed to the new FK (837p: ssn unused→dropped, member_id→existing placeholder fallback; coding-suggest: `*`→just `dob`, the only field used). Fixed 5 JSDoc example comments (`from('patients')`→`from('profiles')`) that the gate's regex was matching. **`patients` now has ZERO prod references** (only a manual integration test remains — gate-excluded, flagged for the live-integration work). Added `_shared/auditLogger.ts` to the forbidden-pattern hook's console-check allowlist (it's the logger's own legit transport). gate exit 0; scoped tsc 0/0; lint 0/0; affected tests 39/39. **Edge fns needing redeploy:** generate-837p, sdoh-coding-suggest (+ the 5 patientContext fns from the prior batch).
- **2026-06-11 (sixth) — A2 `patients`: 7 of 8 caller-files done; only encounterService remains (needs Maria decision).** Worked through the 5 remaining callers, routing each to its real owning table (anti-corruption-per-context). **Done + committed + live-proven:** batchCalculation (profiles + role IN patient/senior); ai-medication-adherence-predictor/patientContext (profiles + senior_demographics for language) **+ 4 SISTER BUGS** found via rule-#1 grep — the other 4 patientContext.ts edge files selected `date_of_birth`/`sex`/`preferred_language` from profiles (cols are `dob`/`gender`; preferred_language is on senior_demographics) → silently null demographics for 4 clinical AI fns, fixed; eligibilityNode (→ patient_insurance: payer_id/is_active, dropped unused member_id); lawEnforcementService 3 methods (profiles + senior_emergency_contacts; check_ins has NO FK so its `!inner` embed was replaced by a separate latest-check-in query; law_enforcement_response_info embed FK verified). Commits: `058b286a`, `5b839bd9`, `2b82a282`. **5 edge functions need REDEPLOY** (deno not local). **⏳ encounterService — SURFACED to Maria, NOT done:** 7 `patients` refs = 4 DEAD CRUD methods (getPatient/createPatient/updatePatient/searchPatients — zero callers) + 3 reachable PostgREST embeds `patient:patients(...)` (lines 51/82/467 in getEncounter/getEncountersByPatient/list). `encounters.patient_id` has **NO FK** → embeds can't auto-resolve to profiles (need separate-fetch restructure across 3 methods + transformEncounterData). `ssn`/`member_id` exist on NO live table (Patient type marks them optional → droppable, aligns with SS-5 "SSN not persisted"). Decisions needed: (a) restructure the 3 embeds (engineering, doable) vs add a FK (Tier-3 schema); (b) the 4 dead CRUD methods — remove (Tier-3 delete) vs repoint; (c) confirm dropping ssn/member_id from the Patient model (Akima/compliance). The `patients` baseline entry clears only after this.
