...
- 🔴 **NEW — CRITICAL, BLOCKED on decision** `DiagnosticReportService` — **4 of 7 methods call RPCs that DO NOT EXIST in the live DB**: `get_recent_diagnostic_reports`, `get_lab_reports`, `get_imaging_reports`, `get_pending_reports` (verified via pg_proc — all absent). So `getRecent`, `getLabReports`, `getImagingReports`, `getPending` all throw. Decision needed (Maria): (a) create the 4 RPCs via migration, or (b) reimplement the 4 methods as direct table queries. getRecent/getPending are unambiguous to reimplement; getLabReports/getImagingReports need the stored `category` value convention confirmed (FHIR `LAB`/`RAD` vs `laboratory`/`imaging` — Observation uses the latter, so there is a real cross-service vocabulary inconsistency). Table is empty (0 rows) so the value cannot be sampled.
