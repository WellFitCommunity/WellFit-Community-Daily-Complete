...
- **2026-06-06 — CRON-AUTH GAP CLOSED (the ⚠️ STILL OPEN item above), live-proven across a real scheduled run.** The scheduled `guardian-automated-monitoring` cron was 401-ing at the **gateway** (`UNAUTHORIZED_INVALID_JWT_FORMAT`): `trigger_guardian_monitoring()` sends the new-format `sb_secret_*` key (from Vault `sb_secret_key`), which is NOT a JWT, so the platform's default `verify_jwt=true` rejected it before guardian-agent ran. Maria's call (asked + chose the posture): **option (ii) — `verify_jwt=false` + in-function cron-secret auth**, matching the rest of the codebase (`security-alert-processor`, `detect-no-shows`, all MCP servers).
  ...
  - **⚠️ SIBLING STILL OPEN (same root cause, NOT touched here):** `security-alert-processor`'s cron 401s every minute the same way — its cron sends `Bearer <vault cron_secret>` but **Vault has no `cron_secret`** (confirmed: vault holds only `app_encryption_key`, `MAILERSEND_API_KEY`, `sb_secret_key`, SMTP creds). The function already accepts `SB_SECRET_KEY`, so the symmetric one-line fix is to point its cron at `vault.sb_secret_key` (+ add `[functions.security-alert-processor] verify_jwt=false`). Left for Maria's go-ahead — separate function, separate auth path.
