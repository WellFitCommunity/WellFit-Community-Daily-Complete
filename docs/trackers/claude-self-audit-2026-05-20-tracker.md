...
| # | Finding | Description | Files Affected | Est. Hours | Status |
|---|---------|-------------|----------------|-----------|--------|
| CR-1 | Variable shadowing of imports — codebase-wide sweep | Pattern `const X = Deno.env.get("X") ?? X;` where `X` is also imported violates `adversarial-audit-lessons.md` Rule #5. Found in 7+ files (see table below). **Fix:** drop the redundant `const` declaration in each file — the import is already in scope. If you need a different fallback chain, rename the local variable. After fixing, **add a CI gate** (extend `S-CI-2`) that fails when this pattern appears: `grep -rEnB1 "^\s*\?\? ([A-Z_]+)\s*;?\s*$" supabase/functions \| grep -B1 "const \1 ="`. **Affected files:** `realtime_medical_transcription/index.ts:38`, `generate-api-key/index.ts:11+13`, `test-users/index.ts:12`, `admin_start_session/index.ts:9`, `admin_end_session/index.ts:8`, `enrollClient/index.ts:13+15`, `_shared/cors.ts:76`. | 7 edge functions | 2 | TODO |
| CR-2 | Compass Riley JSON regex stripping (pre-existing) | `realtime_medical_transcription/index.ts:380` does `text.replace(/\`\`\`json\s*/g, "")...JSON.parse(cleaned)`. Migrate to Anthropic structured output (`response_format: { type: 'json_schema' }`) when this code is next touched. CLAUDE.md Rule #16. | `supabase/functions/realtime_medical_transcription/index.ts:380` | 1.5 | TODO |
...
| G-1 | Guardian — `SELECT *` on 3 monitoring queries | Lines 191, 198, 204 select all columns. Replace with explicit lists. For `phi_access_logs` especially — only need `id, user_id, records_accessed, accessed_at`. | `supabase/functions/guardian-agent/index.ts:188-209` | 0.5 | TODO |
| G-3 | Guardian — HTML email body not escaped | Lines 322-346 interpolate `alert.title`, `alert.message`, `alert.severity`, `alert.category` directly into HTML email body. **Fix:** HTML-escape each field before interpolation, OR switch to `text` body (no `html` field) in the `send-email` call. | `supabase/functions/guardian-agent/index.ts:322-346` | 0.5 | TODO |
| G-4 | Guardian — `Math.max(...arr)` stack overflow risk | Line 258: `Math.max(...unusualAccess.map((a) => a.records_accessed))`. **Fix:** `unusualAccess.reduce((max, a) => Math.max(max, a.records_accessed), 0)`. | `supabase/functions/guardian-agent/index.ts:258` | 0.25 | TODO |
...
| API-5 | Deprecated `String.prototype.substr` | Line 119: `Math.random().toString(36).substr(2, 9)` — `substr` is deprecated. **Fix:** `substring(2, 11)`. | `src/components/admin/ApiKeyManager.tsx:119` | 0.1 | TODO |
| API-6 | Date.parse aggressive sort | Line 437: `Date.parse(v)` treats any parseable string as a date. Org name `"2024 Healthcare Inc."` would sort as timestamp. **Fix:** stricter date detection (regex match for ISO 8601 first). | `src/components/admin/ApiKeyManager.tsx:437` | 0.5 | TODO |
