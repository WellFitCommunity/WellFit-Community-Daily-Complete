...
> **Scope:** Make both transfer flows work end-to-end for a real user: EMS prehospital → ER (Subsystem A) and hospital ↔ hospital handoff packets (Subsystem B). Subsystems C (Transfer Center) and D (post-acute) are dispositioned in the decision queue, not repaired here.
> **Estimate:** ~3 sessions (16–48h class per `implementation-discipline.md`).
...
| **D5** | Two overlapping hospital↔hospital mechanisms: handoff packets (B) vs Transfer Center `transfer_requests` (C, live tables + 5 RPCs, no UI) | (a) B is go-forward, C stays dormant; (b) converge later | **(a) for this tracker** — repairing B; C untouched (Tables-are-FEATURES: no deletion) | none |
| **D6** | `postAcuteTransferService` (orphaned; reads 3 nonexistent tables + dead profiles cols) | (a) repair against live tables (`medications`, `allergy_intolerances`, `fhir_observations`, `encounter_diagnoses`) + wire into discharge flow; (b) defer to its own tracker | **(b)** — real repair includes discharge-flow UX; do not silently half-fix | none |
...
**Session 3 — EMS writer + closeout** (needs D3; D1 if (a)): E-2 (+ edge fn) + E-6 → E-3 → S-1 → S-2 sweep → full E2E walk of both flows with evidence → PROJECT_STATE update.

**Definition of done for the tracker:** a clinician can (1) click to `/ems`, submit a paramedic handoff, see it on the ER board, acknowledge, mark arrived, transfer-to-ER, and find the patient's encounter + vitals in the chart; (2) click New Transfer, send a packet, and the receiving side can acknowledge + integrate it into a chart — all live, all tenant-stamped, all audit-logged, with Maria's visual acceptance on every new/changed screen.
