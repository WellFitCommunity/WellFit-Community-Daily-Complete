# TypeScript Types Specification - NurseOS Resilience Hub

**File to create:** `src/types/nurseos.ts`

This document specifies all TypeScript types for the Emotional Resilience Hub, aligned with the database schema and FHIR naming conventions.

---

## Core Principle: FHIR-Style Naming

Follow FHIR R4 conventions:
- **PascalCase** for type names (e.g., `ProviderBurnoutAssessment`)
- **snake_case** for database fields (matches SQL schema)
- **Optional fields** marked with `?` if nullable in database
- **Generated columns** marked as `readonly` or optional
...
// This file should export all types, interfaces, and utilities
// Import in other files: import { ProviderBurnoutAssessment, ... } from '@/types/nurseos';
```
...
```typescript
// src/types/nurseos.ts (main file)
export * from './nurseos/burnout';
export * from './nurseos/checkins';
export * from './nurseos/training';
export * from './nurseos/support';
export * from './nurseos/config';
export * from './nurseos/utils';
```

**OR** keep as single file (recommended for MVP) to reduce complexity.
...
```typescript
import { ProviderBurnoutAssessment, ResilienceHubApiResponse } from '@/types/nurseos';

export async function submitBurnoutAssessment(
  data: Partial<ProviderBurnoutAssessment>
): Promise<ResilienceHubApiResponse<ProviderBurnoutAssessment>> {
  try {
...
- [x] All database columns have corresponding TypeScript fields
- [x] Optional fields (`?`) match nullable columns
- [x] Enums match database CHECK constraints
- [x] Date/timestamp fields use ISO 8601 strings (not Date objects, for JSON serialization)
- [x] Foreign keys are typed as `string` (UUIDs)
- [x] JSONB fields have typed interfaces (e.g., `MBIQuestionnaireResponse[]`)
- [x] Generated/computed columns marked as optional or readonly
- [x] Type guards for runtime validation
- [x] Utility functions for calculations (composite score, risk level)

---

## Next Steps for Implementation

1. **Create the file:** `src/types/nurseos.ts`
2. **Copy the full type definitions** from this document
3. **Add to index:** `export * from './nurseos';` in `src/types/index.ts`
4. **Validate:** Run `npm run typecheck` to ensure no errors
5. **Use in service layer:** Import types in `resilienceHubService.ts`
6. **Use in components:** Import types in React components
